...
const int M2 = 7;
 ...
 // detachInterrupt(digitalPinToInterrupt(2));//Disable interrupt when calculating
  rpm = half_revolutions * 60/2*2; // Convert frecuency to RPM, note: this works for one interruption per full rotation. For two interrups per full rotation use half_revolutions * 30.
